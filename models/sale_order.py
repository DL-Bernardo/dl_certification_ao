...
class SaleOrderLine(models.Model):
    ...
    def unlink(self):
        for line in self:
            if line.order_id.state == 'draft' and not line.order_id.certificated:
                return super(SaleOrderLine, self).unlink()
            else:
                raise ValidationError(_('Aviso\n Apenas é possível eliminar linhas de documentos '
                                        'no estado de rascunho e não certificados.'))

    # proibir alterar linhas em estado diferente de rascunho
    def write(self, vals):
        for line in self:
            if line.order_id.state != 'draft':
                raise ValidationError(_('Aviso\n Apenas é possível alterar linhas de documentos '
