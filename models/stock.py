            ...
            if len(pick.move_ids) > 0:
                ...
                if not pick.hash:
                    ...
                    self.env.cr.execute("""
                                        ...
                                        FROM res_company c, res_company_users_rel rcu
                                        WHERE c.id=rcu.cid and rcu.user_id=%s""", (self.env.user.id,))
                    nome_emp = self.env.cr.fetchone()
                    ...
                    identi = nome_emp + str(self.env.user.id) + "sp"
