...
class AccountPayment(models.Model):
    ...
    def create_hash(self):
        for payment in self:
            ...
            nome_emp = payment.company_id.create_date
            for key in [" ", ".", ":", "-"]:
                nome_emp = str(nome_emp).replace(key, "")

            identi = nome_emp + str(self.env.user.id)
            ...
            values = hash_generation.hash(self, payment.journal_id.integrado, payment.journal_id.manual, datadocumento,
                ...
                if needs_atcud == 'True':
                    ...
                    if not codigo_validacao_serie:
                        if self.env.user.has_group('account.group_account_manager'):
                            # Formerly 'opc_certification.action_ir_sequence_atcud'
