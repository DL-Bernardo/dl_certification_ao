...
class AccountMove(models.Model):
    ...
    def validar_hash(self):
        for invoice in self:
            #Garante que estamos a encontrar o documento anterior com base na ordem cronológica real
            domain_invoice = [
                ('hash', '!=', False),
            ...
            # Ordenar pela data do documento e número sequencial
            invoices_anteriores = self.search(domain_invoice, order='invoice_date, hash_date, id')

            # Procurar o último documento com hash válido
            antigoHash = ''
            for inv in invoices_anteriores:
                if inv.hash:
                    antigoHash = inv.hash

            # Criar identificador único (como já usavas)
            nome_emp = invoice.company_id.create_date
            for key in [" ", ".", ":", "-"]:
                nome_emp = str(nome_emp).replace(key, "")
            identificador = nome_emp + str(self.env.user.id) + str(invoice.id)

            return int(len(invoices_anteriores)), antigoHash
    ...
    def _get_sequence(self):
        ...
        if self.move_type in ('entry', 'out_invoice', 'in_invoice', 'out_receipt', 'in_receipt') or not journal.refund_sequence:
            if not journal.sequence_id:
                raise UserError(_('Por favor defina uma sequência no diário "%s".') % journal.display_name)
