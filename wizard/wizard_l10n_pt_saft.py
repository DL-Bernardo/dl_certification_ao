...
productCompanyTaxID = '5002601745'
productID = 'InvoiceOne/DIGITALUB ANGOLA - PRESTAÇAO DE SERVIÇOS E COMERCIO GERAL (SU) LDA'
productVersion = 'DL - 17.0'
headerComment = 'Software criado por Odoo e adaptado por DIGITALUB ANGOLA'
softCertNr = '000/AGT/2025'
...
class WizardSaft(models.Model):
    ...
    def act_getfile(self):
        ...
        xml_txt = et.tostring(root, encoding="utf-8", xml_declaration=True, pretty_print=True) #Muito Importante
        _logger.info("\n========= XML GERADO =========\n%s", xml_txt.decode("utf-8"))
    ...
    def _get_masters(self):
        ...
        # 2.2 Customer test vscod
        _logger.info("saft :", ' A exportar Clientes')
        self._write_partners(master, partner_role='Customer')
    ...
    def _write_partners(self, master, partner_role='Customer'):
        ...
        # Coleta todos os parceiros de guias de remessa primeiro
        stock_picking_partners = self.env["stock.picking"].sudo().search([
        ...
        for partner in parceiros_faturas + parceiros_pagamentos + parceiros_guias \
            ...
            # Garante inclusão do "Consumidor Final"
            res_partner = self.env['res.partner'].search([
                ('name', '=', 'Consumidor Final'),
                ('company_id', '=', self.comp.id)], limit=1)
            if res_partner and res_partner.id not in partner_ids:
        ...
        for partner in self.env['res.partner'].search([('id', 'in', partner_ids), '|', ('active', '=', True),
            ...
            # CORREÇÃO: Lógica para SupplierTaxID/CustomerTaxID
            partner_tax_id = '999999999'  # Valor padrão
            if partner.vat:
                partner_tax_id = re.sub("[^0-9]", "", partner.vat)
            elif partner_role == 'Supplier':  # Apenas para fornecedores
                # Tentar obter o NIF de outro campo se vat estiver vazio
                if hasattr(partner, 'nif') and partner.nif:
                    partner_tax_id = re.sub("[^0-9]", "", partner.nif)

            et.SubElement(partner_element, '%sTaxID' % partner_role).text = partner_tax_id
    ...
    def _write_invoice(self, invoice, eparent, versao):
        ...
        # 4.1.4.6  InvoiceDate
        et.SubElement(eparent, u"InvoiceDate").text = str(invoice.date)
        # 4.1.4.7 InvoiceType
        if invoice.move_type == 'out_refund':
        ...
        for payments in pagamentoscliente:
            ...
            for rec in payments.move_id._get_reconciled_invoices_partials():
                ...
                if inv.move_type != 'entry':
                    ...
                    if account_invoice_tax:
                        ...
                        # 4.4.4.14.6.6
                            ...
                            if tax.exemption_reason is not None and tax.exemption_reason != '' and \
                                ...
                                tax_exemption_code = tax.description.split(' ')[0]
                    ...
                    # 4.4.4.15.2
                    et.SubElement(document_totals, 'NetTotal').text = "{:.2f}".format(payments.amount)
                    # 4.4.4.15.3
                    et.SubElement(document_totals, 'GrossTotal').text = "{:.2f}".format(payments.amount)
        ...
        if True:
            ...
            for picking in stock_picking:
                ...
                if picking.write_date is not None:
                    data = str(picking.write_date)[:19].replace(' ', 'T')
                else:
                    data = str(picking.create_date)[:19].replace(' ', 'T')
                ...
                if picking.date_done:
                    et.SubElement(stock_movement, 'MovementEndTime').text = str(picking.date_done).replace(' ', 'T')
                else:
                ...
                for linha_move in picking.move_ids:
                    ...
                    # 4.2.3.20.9
                    #et.SubElement(line, 'DebitAmount').text = ''  # as nossas guias não são valorizadas
                    # GPT
                    et.SubElement(line, 'CreditAmount').text = '1.00'
        ...
        if sale_order:
            ...
            for sale in sale_order:
                        ...
                        for tax in sale_line.tax_id:
                            ...
                            if tax.saft_tax_type == 'IVA' and tax.amount == 0.0:
                                if not tax.exemption_reason or len(tax.exemption_reason) < 6:
                                    ...
                                    et.SubElement(eline, u"TaxExemptionReason").text = str(tax.exemption_reason)
                                    TaxExemptionCode = tax.description.split(' ')[0]
                ...
                if line_no != 0:
                    ...
                    etax_payable.text = "{:.2f}".format(sale.amount_tax) or '0.0'
                    # 4.3.4.15.2. (NetTotal)*
                    enet_total = et.SubElement(edocument_totals, u"NetTotal")
                    enet_total.text = sale.amount_untaxed and "{:.2f}".format(sale.amount_untaxed / cambio) or '0.0'
    ...
    def _write_source_documents(self, start_date, final_date, versao, empresa):
        ...
        # Separar faturas e notas
        faturas = [inv for inv in invoices if inv.move_type == 'out_invoice']
        notas = [inv for inv in invoices if inv.move_type == 'out_refund']

        # Ordenar cada grupo
        faturas_ordenadas = sorted(faturas, key=lambda inv: (
            inv.invoice_date,
            inv.hash_date or inv.create_date,
            extract_number(inv)
        ))

        notas_ordenadas = sorted(notas, key=lambda inv: (
            inv.invoice_date,
            inv.hash_date or inv.create_date,
            extract_number(inv)
        ))

        # Unir: faturas primeiro, depois notas
        invoices = faturas_ordenadas + notas_ordenadas
            ...
            for invoice in invoices:
                if invoice.date >= start_date and invoice.hash is not None:
                    if final_date is False or invoice.date <= final_date:
                        ...
                        if invoice.move_type == 'out_refund':
                            invoices_same_series = [inv for inv in invoices if inv.move_type == 'out_refund']
                            index = invoices_same_series.index(invoice)
                            numHash = index
                            antigoHash = ''
                            if index > 0:
                                antigoHash = invoices_same_series[index - 1].hash
                        else:
                            numHash = 0
                            antigoHash = ''

                        if not invoice.hash:
                            values = invoice.generate_hash(
                                integrado=False,
                                manual=False,
                                datadocumento=invoice.date,
                                datasistema=invoice.write_date or fields.Datetime.now(),
                                number=invoice.name,
                                identi=invoice.internal_number,
                                numHash=numHash,
                                antigoHash=antigoHash,
                                totalbruto=invoice.grosstotal()
                            )
                        ...
                        # 4.1.4.19.3
                        egross_total = et.SubElement(edocument_totals, u"GrossTotal")
                        etax_payable.text = invoice.amount_tax and "{:.2f}".format(invoice.amount_tax / cambio) or '0.0'
                        ...
                        if invoice.currency_id.name != 'AOA':
                            ...
                            ecurrency_credit_amount.text = "{:.2f}".format(invoice.grosstotal())
                            ecurrency_debit_amount.text = str(round(cambio, 4))
